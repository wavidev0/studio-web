...
export const createStudio = createAsyncThunk(
  "admin/doctor/create",
  async (payload: AllUsersPayload | undefined) => {
    return axios.post(`admin/doctor/create`, payload?.data, {
...
const doctorSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(createStudio.fulfilled, (state, action: any) => {
      state.isLoading = false;
      if (action.payload?.data?.status) {
        state.doctor.unshift(action.payload.data.data);
        state.total += 1;
    ...
    builder.addCase(doctorActionAccepted.fulfilled, (state, action: any) => {
      ...
      Success("Doctor Request Accepted Succesfully");
    });
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<any>) => {
        if (
          ...
          action?.payload?.data?.data?._id !== "683942f2e531755412583500"
        ) {
    ...
    builder.addCase(blockDoctor.fulfilled, (state: any, action: any) => {
      if (action?.payload?.status) {
        ...
        action.payload.data?.isBlock === true
          ? Success("Doctor Block Successfully")
          : Success("Doctor Unblock Successfully");
      }
      state.isLoading = false;
    });
