...
apiInstance.interceptors.request.use(
  (config: AxiosRequestConfig): any => {
   ...
   // Read key from global axios defaults (set at login via SetDevKey)
   const devKey = (axios.defaults.headers.common as any)["key"] || key;
   (config.headers as any).key = devKey;
    return config;
  },
